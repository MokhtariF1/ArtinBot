                            ...
                            if session == "Practice_1" or session == "Practice_2" or session == "Practice_3" or session == "Qualifying" or session == "Race":
                                ...
                                if user_response.data == b'summary':
                                    ...
                                    if find_reply is None:
                                        ...
                                        # ask user for video quality with inline keys in conversation
                                        avalable_quality = find_reply["summary"].keys()
                                        quality_buttons = [
                                            [
        ...
        #             await event.reply(bot_text["successfully"])
            ...
            async with bot.conversation(user_id, timeout=1000) as conv_all:
                ...
                if response.data == b'rs':
                    ...
                    count_reply = watch_reply_collection.find_one({"user_id": user_id})
                    if count_reply is not None:
                        count_reply = len(watch_reply_collection.find_one({"user_id": user_id})["watched"]) + 1
                    else:
                        count_reply = 0
                    first_text = bot_text["reply_count"]
                    requests_reply = watch_reply_collection.find_one({"user_id": user_id})["watched"]
                    races = "\n"
                    summray = "\n"
                    drivers_requests = "\n"
                    fastest = "\n"
                    for request in requests_reply:
                        request, quality = reply_collection.find_one(request[0]), request[1]
                        session = None
                        gp = None
                        if lang == 1:
                            session = sessions_convert[request["event"]]
                            gp = request["gp"]
                        else:
...
@bot.on(events.CallbackQuery(pattern="summary_reply:*"))
async def summary_reply(event):
    # find reply in reply collection and then show user avalable qualitys
    find_id = event.data.decode().split(":")[1]
    find_reply = reply_collection.find_one({"_id": ObjectId(find_id)})
    if find_reply is None:
        ...
        qualitys = []
        for k in find_reply["summary"].keys():
            qualitys.append([Button.inline(k, str.encode("summary_get_video:" + find_id + ":" + k))])
        await event.reply(bot_text["select_quality"], buttons=qualitys)
    ...
    if find_reply is None:
        ...
        if find_watch is None:
            watch_reply_collection.insert_one(
                {
                    "user_id": user_id,
                    "watched": [(ObjectId(find_id), quality)],
                }
            )
        else:
            watch_reply_collection.update_one({"user_id": user_id}, {"$push": {"watched": (find_reply["_id"],quality)}})
        await event.reply(bot_text["delete_video_warn"])
        for i in range(20):
            await asyncio.sleep(1)
        await bot.delete_messages(user_id, s.id)
@bot.on(events.CallbackQuery(pattern="summary_get_video:*"))
async def summary_get_video(event):
    # get video from link and send it to user
    user_id = event.sender_id
    find_id = event.data.decode().split(":")[1]
    quality = event.data.decode().split(":")[2]
